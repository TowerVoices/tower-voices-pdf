  ...
  return (
    <div dir="rtl" className="fixed inset-0 z-[9999] bg-[#0a0a0a] flex flex-col font-sans">
      <header className="h-16 bg-black border-b border-white/10 flex items-center justify-between px-6 z-20 shadow-xl">
          ...
          <h1 className="text-white font-bold text-sm md:text-base truncate max-w-[200px] md:max-w-md text-center">{title}</h1>
      ...
      <main className="flex-1 relative overflow-hidden bg-[#151515] flex justify-center p-4">
        <div className="h-full w-full max-w-5xl bg-white shadow-2xl rounded-sm overflow-hidden border border-white/5">
            {/* توحيد الإصدار إلى 3.4.120 لحل الخطأ الأحمر */}
            <Worker workerUrl="https://unpkg.com/pdfjs-dist@3.4.120/build/pdf.worker.min.js">
                <Viewer
                    fileUrl={pdfUrl}
                    // تم تغيير initialScale إلى defaultScale لإصلاح خطأ TypeScript
          ...
          >
            <section className="space-y-6">
              ...
              <div className="flex items-center justify-between bg-white/5 p-2 rounded-2xl">
                <ZoomOut>{(props) => <button onClick={props.onClick} className="p-4 hover:bg-white/10 rounded-xl text-white transition-all"><FaMinus /></button>}</ZoomOut>
                <div className="text-blue-400 font-black text-xs">
                  <CurrentScale>{(props) => <>{Math.round(props.scale * 100)}%</>}</CurrentScale>
                </div>
                <ZoomIn>{(props) => <button onClick={props.onClick} className="p-4 hover:bg-white/10 rounded-xl text-white transition-all"><FaPlus /></button>}</ZoomIn>
              </div>
            </section>
            ...
            <section className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-3">
                <button onClick={() => setViewMode(ViewMode.SinglePage)} className={`p-4 rounded-2xl flex flex-col items-center gap-2 text-[10px] border transition-all ${viewMode === ViewMode.SinglePage ? "bg-blue-600 border-blue-400 text-white" : "bg-white/5 border-transparent text-gray-500 hover:bg-white/10"}`}><FaRegFileAlt size={20}/> صفحة واحدة</button>
                <button onClick={() => setViewMode(undefined)} className={`p-4 rounded-2xl flex flex-col items-center gap-2 text-[10px] border transition-all ${viewMode === undefined ? "bg-blue-600 border-blue-400 text-white" : "bg-white/5 border-transparent text-gray-500 hover:bg-white/10"}`}><FaThList size={20}/> مستمر</button>
              </div>
            </section>
          </motion.aside>
        )}

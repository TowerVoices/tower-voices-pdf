  ...
  return (
    <div dir="rtl" className="fixed inset-0 z-[9999] bg-[#0a0a0a] flex flex-col font-sans select-none">
      ...
      <header className="h-14 bg-black border-b border-white/5 flex items-center justify-between px-6 z-[10001] shadow-2xl">
          {/* تعديل الزر ليقوم بفتح وإغلاق القائمة (Toggle) */}
          <button 
          ...
          <h1 className="text-gray-200 font-bold text-xs md:text-sm truncate max-w-[180px] md:max-w-md text-center opacity-80">
            {title}
          </h1>
      ...
      {/* 2. منطقة القراءة (توسيط الصفحة البيضاء) */}
      <main className="flex-1 relative overflow-hidden bg-[#111111] flex justify-center p-2 md:p-6">
        <div className="h-full w-full max-w-4xl bg-white shadow-[0_0_60px_rgba(0,0,0,0.7)] overflow-hidden rounded-sm border border-black/20">
            {/* مطابقة الإصدار 3.4.120 بدقة لحل خطأ الهيكل */}
      ...
      <AnimatePresence>
        {showControls && (
          <motion.aside 
            ...
            className="fixed top-14 right-0 bottom-0 w-72 bg-[#0d0d12]/95 backdrop-blur-3xl border-l border-white/5 p-8 flex flex-col gap-10 z-[10000] shadow-[-30px_0_60px_rgba(0,0,0,0.9)]"
          >
            <section className="space-y-6">
              <h4 className="text-[10px] text-gray-500 font-bold uppercase tracking-[0.2em] px-1">التحكم في التكبير</h4>
              <div className="flex items-center justify-between bg-white/5 p-1.5 rounded-2xl border border-white/5">
                <ZoomOut>{(props) => (
                  <button onClick={props.onClick} className="p-4 hover:bg-white/10 rounded-xl text-gray-400 hover:text-white transition-all"><FaMinus /></button>
                )}</ZoomOut>
                
                <div className="text-blue-500 font-black text-[11px] tracking-tighter">
                  <CurrentScale>{(props) => <>{Math.round(props.scale * 100)}%</>}</CurrentScale>
                </div>

                <ZoomIn>{(props) => (
                  <button onClick={props.onClick} className="p-4 hover:bg-white/10 rounded-xl text-gray-400 hover:text-white transition-all"><FaPlus /></button>
                )}</ZoomIn>
              </div>
            </section>

            <section className="space-y-6">
              <h4 className="text-[10px] text-gray-500 font-bold uppercase tracking-[0.2em] px-1">نمط الصفحات</h4>

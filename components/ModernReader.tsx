...
// حذف استيراد defaultLayoutPlugin تماماً
...
export default function ModernReader({ pdfUrl, title, onClose }: ModernReaderProps) {
  ...
  // تفعيل ملحق الزووم لربطه بأزرارك المخصصة في اللوحة الجانبية
  const zoomPluginInstance = zoomPlugin();
  const { ZoomIn, ZoomOut, CurrentScale } = zoomPluginInstance;

  // معالجة الرابط عبر الجسر البرمي لضمان عمل Google Drive
  ...
  return (
    <div dir="rtl" className="fixed inset-0 z-[9999] bg-[#0a0a0a] flex flex-col font-sans select-none">
      ...
      {/* 2. منطقة القراءة (توسيط الملف وإخفاء الأدوات الافتراضية) */}
      <main className="flex-1 relative overflow-hidden bg-[#111111] flex justify-center items-center p-2 md:p-6">
        ...
        {isLoading && (
          <div className="absolute inset-0 z-10 flex flex-col items-center justify-center bg-[#111111] gap-4">
            ...
            <p className="text-[10px] text-gray-500 tracking-widest font-bold font-sans">جاري تحميل الرواية...</p>
          </div>
        )}

        <div className="h-full w-full max-w-5xl bg-white shadow-[0_0_50px_rgba(0,0,0,0.5)] overflow-hidden rounded-sm border border-black/10">
            {/* استخدام نسخة Worker المطابقة 3.4.120 */}
            <Worker workerUrl="https://unpkg.com/pdfjs-dist@3.4.120/build/pdf.worker.min.js">
                {finalPdfUrl && (
                  <Viewer
                    fileUrl={finalPdfUrl}
                    defaultScale={SpecialZoomLevel.PageWidth}
                    // التغيير الجوهري: نستخدم فقط zoomPluginInstance
          ...
          >
            ...
            <section className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-3">
                <button onClick={() => setViewMode(ViewMode.SinglePage)} className={`p-4 rounded-2xl flex flex-col items-center gap-3 text-[10px] border transition-all ${viewMode === ViewMode.SinglePage ? "bg-blue-600 border-blue-400 text-white shadow-lg" : "bg-white/5 border-transparent text-gray-500 hover:bg-white/10"}`}><FaRegFileAlt size={20}/> صفحة</button>
                <button onClick={() => setViewMode(undefined)} className={`p-4 rounded-2xl flex flex-col items-center gap-3 text-[10px] border transition-all ${viewMode === undefined ? "bg-blue-600 border-blue-400 text-white shadow-lg" : "bg-white/5 border-transparent text-gray-500 hover:bg-white/10"}`}><FaThList size={20}/> مستمر</button>
              </div>
            </section>
          </motion.aside>
        )}
